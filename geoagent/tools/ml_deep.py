...
def train_mlp_classifier(
    ...
    Xt = torch.tensor(Xn[tr]); yt = torch.tensor(y_i[tr])
    Xv = torch.tensor(Xn[va]); yv = torch.tensor(y_i[va])

    best, best_state, bad = float("inf"), None, 0
    curve = {"train": [], "val": []}
    for epoch in range(max(5, epochs)):
        net.train()
        ...
        with torch.no_grad():
            ...
            vacc = float((net(Xv).argmax(1) == yv).float().mean())
    ...
    payload = {
        ...
        "hidden": list(hidden or [64, 32]), "mu": mu.tolist(), "sigma": sigma.tolist(),
        "classes": [float(c) for c in classes], "val_acc": acc, "epochs_run": epoch + 1,
    }
    ...
    return (
        f"训练完成: {model_name} (轮数 {epoch + 1}, 早停 patience={early_stopping})\n"
