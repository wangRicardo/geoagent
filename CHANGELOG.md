...
- 软件内厂商管理：设置窗口（⚙ 设置）添加/编辑/删除自定义厂商（API 地址+密钥+默认模型），
  多厂商并存一键切换；内置厂商也可在软件内直接保存密钥（不再必须配环境变量）
- 连接测试按钮：一键验证 API 地址与密钥（拉取 /models）
- 权限管理模式（类似 Claude Code/Codex）：
  - readonly 只读：仅检索/分析/计算，禁止写入、执行、联网
  - standard 标准：工作区内读写+执行+联网（默认）
  - ask 谨慎：写入/执行/联网逐次弹窗确认
  - full 自主：全部允许，无人值守
  工具调用按风险分级（联网/执行/写入/读取）拦截，被拒时反馈给模型自行调整
- 界面重排：模型/思考/权限下拉移到输入框旁（贴手），工作区芯片同排；⚙ 设置入口在顶栏
- CLI 新命令：/mode /key /providers /provider-add
